    ...
    private func _startLocked() {
        ...
        task = sfRecognizer.recognitionTask(with: req) { [weak self] result, error in
            ...
            if let result {
                ...
                if !text.isEmpty && (text != prev || isFinal) {
                    let prefix = isFinal ? "F:" : "P:"
                    print("\(prefix)\(text)")
                    fflush(stdout)
                    self.lock.lock()
        ...
        recognQ.asyncAfter(deadline: .now() + 4) { [weak self] in
            ...
            if stuck {
                ...
                self.lock.lock(); self.useOnDevice = true; self.lock.unlock()
                currentTask?.cancel()   // 触发 error 回调 → 重启走端上
            }
        }

        // 45 秒会话上限（端上无此限制，但统一处理）
...
// ── 权限请求 ──────────────────────────────────────────────────────────────────
fputs("DEBUG: 1 启动，请求 Speech 授权…\n", stderr)
let speechSema = DispatchSemaphore(value: 0)
SFSpeechRecognizer.requestAuthorization { status in
    fputs("DEBUG: 2 授权回调 status=\(status.rawValue)\n", stderr)
...
fputs("DEBUG: 5 请求 SCShareableContent…\n", stderr)
SCShareableContent.getWithCompletionHandler { content, error in
    fputs("DEBUG: 6 SCShareableContent 回调\n", stderr)
    guard let display = content?.displays.first else {
        fputs("ERROR: 无法获取屏幕内容（\(error?.localizedDescription ?? "unknown")）\n", stderr)
        exit(1)
    }
    fputs("DEBUG: 7 已获取 display，准备创建 SCStream…\n", stderr)
    ...
    do {
        ...
        fputs("DEBUG: 8 已 addStreamOutput，调用 startCapture…\n", stderr)
        stream.startCapture { err in
            fputs("DEBUG: 9 startCapture 回调\n", stderr)
            if let err {
                fputs("ERROR: \(err)\n", stderr)
                exit(1)
            }
